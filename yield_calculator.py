...
def now_yield(stock_code, language='en'):
    ...
    date = dom.xpath('//*[@id="main-2-QuoteDividend-Proxy"]/div/section[2]/div[2]/div/div/div[2]/ul/li[1]/div/div[5]')[0].text
    ...
    if language == 'zh-TW':
        print(stock_code + "  日期:" + date + ", 當前股價:" + str(share_price) + ", 現金殖利率:" + str(round(cash_dividend * 100 / share_price, 1)) + "%, 股票殖利率:" + str(round(stock_dividend * 100 / share_price, 1)) + "%\n")
    elif language == 'zh-CN':
        print(stock_code + "  日期:" + date + ", 当前股价:" + str(share_price) + ", 现金殖利率:" + str(round(cash_dividend * 100 / share_price, 1)) + "%, 股票殖利率:" + str(round(stock_dividend * 100 / share_price, 1)) + "%\n")
    elif language == 'jp':
        print(stock_code + "  日にち:" + date + ", 現在の株価:" + str(share_price) + ", 配当利回り:" + str(round(cash_dividend * 100 / share_price, 1)) + "%, 株式利回り:" + str(round(stock_dividend * 100 / share_price, 1)) + "%\n")
    elif language == 'kr':
        print(stock_code + "  날짜:" + date + ", 현재 주가:" + str(share_price) + ", 현금수익률:" + str(round(cash_dividend * 100 / share_price, 1)) + "%, 주식 수익률:" + str(round(stock_dividend * 100 / share_price, 1)) + "%\n")
    else:
        print(stock_code + "  Date:" + date + ", Current share price:" + str(share_price) + ", Cash yield:" + str(round(cash_dividend * 100 / share_price, 1)) + "%, Stock yield:" + str(round(stock_dividend * 100 / share_price, 1)) + "%\n")
